...
public:
	...
	time_type getPauseDuration() const { return m_pauseDuration; }
...
{
	std::cout << chcl::Formatter::Format("{0}- {1}: {2} ({3})\n", std::string(depth * 2, ' '), m_scopeName, m_duration, m_pauseDuration);
...
_ProfileScope::~_ProfileScope()
{
	for (auto &[name, childPtr] : m_childScopes)
	{
		delete childPtr;
	}
}
...
_ProfileScope* _ProfileScope::addChildScope(const char *name)
{
	...
	if (child == nullptr)
	{
		child = new _ProfileScope(name);
		child->m_parentScope = this;
		m_childScopes[name] = child;
	}
